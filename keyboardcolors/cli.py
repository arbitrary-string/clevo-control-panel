"""Command-line control of the keyboard backlight, sharing the same backend
as the GUI app. Also used internally by the boot-persistence systemd units
(`save-state` / `restore-state`) so they don't need to know backend details."""
...
def die(msg):
    print(f"keyboardcolors-cli: {msg}", file=sys.stderr)
    sys.exit(1)
...
def _write(fn, *args):
    try:
        ...
        die(
            ...
            "'kbdlight' group, or run this command with sudo."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Control the keyboard RGB backlight (system76_acpi or clevo-acpi)."
    )
    ...
    p.set_defaults(func=cmd_effect_cycle)
    ...
    p.set_defaults(func=cmd_effect_breathe)

    p = effect_sub.add_parser("wave", help="moving rainbow across zones (multi-zone only)")
    ...
    p.set_defaults(func=cmd_effect_wave)
